...
printf "\n[3/5] Invalid login (should be 401)...\n"
curl -si -H "Origin: $ORIGIN" -H "Content-Type: application/json" \
  -d '{"username":"admin","password":"wrong"}' "$API/api/auth" | sed -n '1,80p'

printf "\n[4/5] Valid login (set cookie)...\n"
curl -si -H "Origin: $ORIGIN" -H "Content-Type: application/json" -c "$COOKIE_FILE" \
  -d '{"username":"admin","password":"admin"}' "$API/api/auth" | sed -n '1,120p'

printf "\n[5/5] Authenticated /api/reservations (1 week range)...\n"
